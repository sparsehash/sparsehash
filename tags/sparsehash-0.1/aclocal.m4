...
# Figures out where hash_fun (or stl_hash_fun) is defined, and then
# writes out the location to the file specified in $1.
# The output file also #defines the namespace that hash_fun is in
# HASH_NAMESPACE.
#
# You can use this *instead of* stl_hash_fun.ac.  stl_hash_fun.ac will
# also define vars indicating where hash_fun lives, but won't actually
...
#
# We use our knowledge of configure to know that the $ac_header holds
# the header that's found when the conditional text executes.  We make
# use of that fact, which perhaps we shouldn't...

# Ideally we'd use AC_CACHE_CHECK, but we want to call AC_CHECK_HEADERS which
# prints messages itself, so instead we implement AC_CACHE_CHECK manually
# via AC_MSG_CHECKING + AC_CACHE_VAL + AC_MSG_RESULT

AC_DEFUN([AC_CXX_STL_HASH_FUN],
  [AC_REQUIRE([AC_CXX_NAMESPACES])
   AC_LANG_SAVE
   AC_LANG_CPLUSPLUS
   AC_CHECK_HEADERS(stl_hash_fun.h ext/stl_hash_fun.h \
                    ext/hash_fun.h hash_fun.h,
                    [ac_cv_cxx_stl_hash_fun="<$ac_header>"; break])
   AC_LANG_RESTORE
...
# iterator tags, which are typedefs that say what the iterator can do
# (just ++, or random access, or what).
#
# Unfortunately, I don't know how to use $ac_cv_cxx_stl_namespace from
# stl_namespace.m4, so I just re-implement it.  Ugh.
 
AC_DEFUN([AC_CXX_STL_ITERATOR_TAGS],
  [AC_CACHE_CHECK(
      whether STL supports iterator-tags,
      ac_cv_cxx_stl_iterator_tags,
      [AC_REQUIRE([AC_CXX_STL_NAMESPACE])
       AC_LANG_SAVE
       AC_LANG_CPLUSPLUS
       AC_TRY_COMPILE([#include <list>],
                      [list<int>::iterator::iterator_category c; return 0;],
                      ac_cv_cxx_stl_iterator_tags=yes)
       AC_TRY_COMPILE([#include <list>],
                      [std::list<int>::iterator::iterator_category c; return 0;],
                      ac_cv_cxx_stl_iterator_tags=yes)
       AC_LANG_RESTORE])
   if test "$ac_cv_cxx_stl_iterator_tags" = yes; then
     AC_DEFINE(UNDERSTANDS_ITERATOR_TAGS, 1,
